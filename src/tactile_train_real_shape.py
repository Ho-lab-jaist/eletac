...
from torchvision.models import resnet50, ResNet50_Weights, resnet18, ResNet18_Weights, vgg16, VGG16_Weights
import config
...
def load_model_train_resnet_pre(model_name, lr, num_classes):
    ...
    MODEL_PATH = config.MODEL_PATH / model_name
    tacnet = resnet50()
    tacnet.fc = nn.Linear(tacnet.fc.in_features, num_classes)
    print('model [TacNet] was created')
    print('loading the model from {0}'.format(MODEL_PATH))
    tacnet.load_state_dict(torch.load(MODEL_PATH))
    print('---------- Tactile Networks initialized -------------')
    tacnet.to(dev)
    tacnet.train()
    opt = torch.optim.Adam(tacnet.parameters(), lr=lr, weight_decay=0.01)
    return tacnet, opt, epoch_number
...
def get_model_resnet_pre(lr):
    ...
    #
    # # Optionally, add dropout between layers in the classifier for regularization
    # model.classifier[5] = nn.Dropout(p=0.5)  # Dropout after the 2nd last layer
...
def loss_batch(model, loss_func, xb, yb, opt=None):
    ...
    loss_position = loss_func(model(xb).float(), yb.float())
    # print('predict: ', model(xb).float())
    # print('label: ', yb.float())
    # print('Loss: ', loss_position)
    ...
    for epoch in range(epochs):
        ...
        for xb, yb, _ in train_dl:
            ...
            if (batch)%50 == 0:
                ...
                # print('predict: ', model(xb)[0].float())
                # print('label: ', yb[0].float())
...
INIT_PATH = DATA_PATH_SHAPE / "init_pos.csv"  # pos2 means with depth, pos1 without depth
...
SINGLE_INPUT_PATH = DATA_PATH_SHAPE / "train"           # images
SINGLE_LABEL_PATH = DATA_PATH_SHAPE / "shape_train.csv"
...
model, opt, epoch_number = load_model_train_resnet_pre(model_name='shape_real_resnet_pre_tacnet_eletac_02-18-25_00-18_epoch50_lr0.0001.pt',
                                                   lr=learning_rate,
                                                      num_classes= 6)
# model, opt = get_model_resnet_pre(learning_rate)



epochs = 50 # the number of iteration over a training dataset
...
now = datetime.now()
exp_name = "shape_real_resnet_pre_tacnet_eletac_{}_epoch{}_lr{}".format(now.strftime("%m-%d-%y_%H-%M"), epochs+epoch_number, learning_rate)
